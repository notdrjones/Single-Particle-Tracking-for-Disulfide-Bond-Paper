...
# these functions establish different kernels for averaging
#don't need to deal with normalisation during the setup of kernels because I do it later (this avoids the issue of NaN values)
    ...
    for i in range(0,size):
        if i < n+1:
           ...
           kernel[i] = weight
            ...
            kernel[i] = kernel[abs(size-i-1)]
            
    return kernel

# takes the kernel function specified and applies it to the data to get a rolling average
...
def medianfilter_smoothing(dataframe, options):
    ...
    smoothed_data = pd.DataFrame(columns=['x', 'y'])
...
def meanfilter_smoothing(dataframe, options, n):
    ...
    # because kernel name is a string, which can't be passed as a function
    kernel_function = globals()[kernelname]
    smoothed_data = pd.DataFrame(columns=['x', 'y']) 
    
    #establishes a new data frame in which the rolling average of position i+n is assigned to that index
...
def calculate_diff(df, n):
    diff_array= np.array([]).reshape(0,2)
    firsti = int(n - (n-1)/2) - 1
    lasti = int(len(df) - n)
    for i in range(firsti, lasti+1):
        precede = int(i - (n+1)/2)
        succede = int(i + (n+1)/2)
        diff = np.array([df.iloc[succede]['x'] - df.iloc[precede]['x'], df.iloc[succede]['y'] - df.iloc[precede]['y']])
...
def getDstars(dfTrackingFiltered, p, options):
    p_data2 = getpdata(dfTrackingFiltered, p)
    smoothness = options.get('smoothness')
    vsmoothness = int((smoothness - 1)/2)
    p_dataSmoothed2 = meanfilter_smoothing(p_data2, options, vsmoothness)
